...
from .prompts import DEFAULT_PROMPT_VERSION, PROMPT_V1, prompt_manager
...
async def seed_prompt_v1() -> None:
    """Ensure the baseline prompt v1 exists and is active on a fresh DB."""
    existing = await prompt_manager.list_versions()
    if not existing:
        ...
        logger.info("Seeded baseline prompt version %s", DEFAULT_PROMPT_VERSION)
