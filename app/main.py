...
async def seed_prompt_v1() -> None:
    """Ensure the baseline prompt v1 exists, is active, and matches the code.

    Fresh DB: seed it. Existing DB: refresh v1's text in place so baseline
    improvements (like formatting rules) apply without manual migration —
    derived versions (v2+) generated by /improve are never touched.
    """
    ...
    if not existing:
        ...
        await prompt_manager.update_text(DEFAULT_PROMPT_VERSION, PROMPT_V1)
        logger.info("Baseline prompt version %s refreshed", DEFAULT_PROMPT_VERSION)
